...
class ABC(nn.Module):
	...
	self.bert_encoding_fc = nn.Sequential(nn.LayerNorm(768), nn.Dropout(0.1), nn.Linear(768, 300), nn.ReLU(True), nn.LayerNorm(300),)
	...
	if len(data.size()):
	    print("data",data[0])
	    one = torch.tensor(self.bert_tokenizer.encode(data[0], add_special_tokens=True)).unsqueeze(0)
	    for point in data[1:]:
		two = torch.tensor(self.bert_tokenizer.encode(point, add_special_tokens=True)).unsqueeze(0)
		one = torch.cat((one,two), 0)
	    return one
	return
	...
	print("embed",e_a0.size())
	print("ans",a0.size(), a0_l.size())
	...
	#print(raw_out_q)
	#print("q",raw_out_q.size())
	#print("a",raw_out_a0.size())
