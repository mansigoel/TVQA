...
class TVQADataset(Dataset):
    def __init__(self, opt, mode="train"):
        ...
        self.bert_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', max_len=300)
...
def pad_collate(data):
    ...
    all_values = []
    pca = PCA(n_components=0.390625)
    for i, k in enumerate(all_keys):
        if k in text_keys:
	    a,b = pad_sequences(column_data[i])
	    a = model(a)[0]
	    print("model size", a.size())
	    alldata = []
	    a = a.detach().numpy()
            for point in a:
		print(point.shape)
		new = pca.fit_transform(point)
		print(new.shape)
		alldata.append(new)
	    alldata = torch.from_numpy(np.array(alldata)).float().to("cuda:0")
	    print(alldata.size())
            all_values.append((alldata,b))
        elif k == label_key:
